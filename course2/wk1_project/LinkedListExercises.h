...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  while(currentNode) {
    // Check if data is smaller than current node
    if( newData < currentNode->data ) {
      if (currentNode->prev) {
        // Insert newNode in between as: prevNode < newNode < currentNode
        ...
        // Update the head list as: newNode < currentNode
        newNode->next = head_;
        head_->prev = newNode;
        head_ = newNode;
        break;
      ...
      if (currentNode->next) {
        // Continue search data is smaller
        currentNode = currentNode->next;
      }
      else {
        // Insert newNode as this reach tail: currentNode < newNode
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // For case where both list is empty, return empty in this case
  if (left.empty() && right.empty()) {
    return merged;
  }

  // Search thru both list
  while( !left.empty() || !right.empty() ){
    // For case where either one of the list is empty
    if ((!left.empty() && right.empty()) || (left.empty() && !right.empty())) {
      LinkedList<T> * p_empty = (right.empty())? &right : &left;
      LinkedList<T> * p_full = (right.empty())? &left : &right;
      while (!p_full->empty()) {
        if (merged.empty()) {
          merged.pushBack(p_full->front());
        }
        else {
            const auto merged_back = merged.back();
            const auto to_merge = p_full->front();

            if(merged_back <= to_merge) {
              merged.pushBack(to_merge);
            }
            else {
              // Add in between the list
              merged.popBack();
              merged.pushBack(to_merge);
              merged.pushBack(merged_back);
            }
        }
        p_full->popFront();
      }
    }
    // Case where both list is not empty
    else {
      while(!left.empty() && !right.empty()) {
