...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.clear();
  tree.createRoot(4);
  auto Top = tree.getRootPtr();
  auto First = Top->addChild(8);
  auto Second = First->addChild(16);
  First->addChild(23);
  Second->addChild(42);
  Top->addChild(15);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  node_queue.push(rootNodePtr);
  
  while(node_queue.size() > 0) {
    for (auto childPtr : node_queue.front()->childrenPtrs) {
      node_queue.push(childPtr);
    }  
    results.push_back(node_queue.front()->data);
    node_queue.pop();
  }
